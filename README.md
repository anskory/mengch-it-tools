...
| **正则表达式测试** | 在线测试正则表达式，高亮匹配结果，分组提取，常用模板 |
...
| **JSON 转义** | JSON 转义/去转义，支持 Unicode 转换、符号转换等多种功能 |
| **JSON 字段提取** | 解析 JSON 结构，树形浏览字段，快速提取需要的字段值 |
| **时间戳转换** | 时间戳与日期时间格式相互转换，支持秒和毫秒，多种日期格式 |
...
| **Base64 编码解码** | 对文本进行 Base64 编码和解码，支持中英文等多字节字符 |
| **哈希加密** | 支持 MD5、SHA1、SHA256、SHA384、SHA512 等多种哈希算法 |
...
| **颜色选择器** | 颜色选择与格式转换，支持 HEX/RGB/HSL 互转，生成调色板和配色方案 |
...
| 工具 | 说明 |
|------|------|
| **图片压缩** | 本地浏览器压缩图片，支持调整质量和尺寸，支持 JPG/PNG/WebP 输出 |
